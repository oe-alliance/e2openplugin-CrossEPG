...
if crossepg_instroot is False:
    sys.exit(1)
libdir = os.path.join(crossepg_instroot, 'scripts/lib')
sys.path.append(libdir)

# import local modules
# Python 3 removed sgmllib; html.parser is closest equivalent
import html.parser as sgmllib
import scriptlib
...
class main:
    ...
    def __init__(self, confdir, dbroot):
        ...
        self.CHANNELLIST = {}
        # create a dictionary with index = channel ID
        for i in temp:
            self.CHANNELLIST[i[0]] = str(i[1])
    ...
    def do_epg_alias(self):
        ...
        for src_channel in self.CHANNELLIST:
            ...
            if src_channel.count('-') == 1:
                src_chname = src_channel.split('-')[0].strip().lower()
                src_provname = src_channel.split('-')[1].strip().lower()
                ...
                for p in src_sidprovidbyname:
                    if p[1] == src_provname:
                        ...
                        src_epgdb_channel = crossepg.epgdb_channels_get_by_freq(
                            ch_sid[2], ch_sid[1], ch_sid[0]
                        )
                        if not src_epgdb_channel:
                            self.log.log(
                                'Source channel "%s" with SID "%s" has no entry in epgdb, try next'
                                % (src_channel, src_sidbyname)
                            )
                            continue
                        else:
                            break
                ...
                if len(src_sidbyname) == 0:
                    self.log.log(
                        'Source channel "%s" has no SID in lamedb, skip it' % src_channel
                    )
                    continue

            elif src_channel.count('-') == 2:
                tmp = src_channel.split('-')
                src_sidbyname = "%s:xxxxxxxx:%s:%s:x:x" % (
                    tmp[0].strip(),
                    tmp[1].strip(),
                    tmp[2].strip(),
                )
            ...
            src_epgdb_channel = crossepg.epgdb_channels_get_by_freq(
                src_sid[2], src_sid[1], src_sid[0]
            )
            if not src_epgdb_channel:
                self.log.log(
                    'Source channel "%s" with SID "%s" has no entry in epgdb, skip it'
                    % (src_channel, src_sidbyname)
                )
                continue

            self.log.log(
                'Source channel "%s" with SID "%s" found in epgdb, using it'
                % (src_channel, src_sidbyname)
            )
            ...
            for dst in dst_channels:
                ...
                if len(dst_sidbyname) == 0:
                    self.log.log(
                        '   dest. channel "%s" has no SID in lamedb, skip it' % dst
                    )
                    continue
                ...
                for dsid in dst_sidbyname:
                    ...
                    if dst_sid == src_sid:
                        self.log.log(
                            '   dest. channel "%s" is eq. to source, skip it' % dst
                        )
                        continue

                    num_events = 0
                    self.log.log(
                        '   copying EPG data from "%s" to "%s" sid "%s")'
                        % (src_channel, dst, dsid)
                    )
                    self.log.log2video_status(
                        "copy %s -> %s (%d/%d)"
                        % (src_channel, dst, num_events, total_events)
                    )
                    ...
                    while title is not None:
                        ...
                        e_title = crossepg.epgdb_read_description(title).encode("utf-8", "surrogateescape") # hack
                        e_summarie = crossepg.epgdb_read_long_description(title).encode("utf-8", "surrogateescape") # hack 
                        e_countrycode = "%c%c%c" % (
                            title.iso_639_1,
                            title.iso_639_2,
                            title.iso_639_3,
